#ifndef __SLIGHT_HPP__
#define __SLIGHT_HPP__
    ...
    */
    ...
    void run() {
        ...
        for (iter = 0; iter < max_iter; iter++) {
            // // 归一化pheromone
            auto f_pher = nonlinearty(pheromone, adj);
            auto f_norm_pher = row_normalize(f_pher);

            Eigen::MatrixXd trans_pher = pheromone.transpose();
            Eigen::MatrixXd trans_adj = adj.transpose();
            auto b_pher = nonlinearty(trans_pher, trans_adj);
            auto b_norm_pher = row_normalize(b_pher);
